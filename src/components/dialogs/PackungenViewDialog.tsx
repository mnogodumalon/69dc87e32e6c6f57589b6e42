    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Verbleibende Menge (Anzahl Einheiten)</Label>
            <p className="text-sm">{record.fields.verbleibende_menge ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Voraussichtliches Aufbrauch-Datum</Label>
            <p className="text-sm">{formatDate(record.fields.voraussichtliches_aufbrauch_datum)}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
              record.fields.woechentlich_packung ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
            }`}>
              {record.fields.woechentlich_packung ? 'Ja' : 'Nein'}
            </span>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Morgens</Label>
            <Badge variant="secondary">{record.fields.dosierung_morgens_packung?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Mittags</Label>
            <Badge variant="secondary">{record.fields.dosierung_mittags_packung?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Abends</Label>
            <Badge variant="secondary">{record.fields.dosierung_abends_packung?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Nacht</Label>
            <Badge variant="secondary">{record.fields.dosierung_nacht_packung?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
